...
def markup_file(article_file, backlink_full, backlink_list, lang, stopwords):
	article_dirpath = "/scratch-local/users/dhruvils/Part2-TextExtraction/data/articles/%s/%s" %(lang, article_file)
				...
				if word in matched_set:
					...
					if word not in unmatched_set:
						for name in backlink_list:
							if levenshteinDistance(word, name)['ratio'] > 0.75:
								markup_list[i] = tuple([word, "PERS"])
...
file_count = 0
backlinks_path = "/scratch-local/users/dhruvils/Part2-TextExtraction/data/backlinks"
# backlinks_path = "/scratch-local/users/dhruvils/Part3-NamesMarkup/data/backlinks"
(backlink_full, backlink_set) = get_all_backlinks(backlinks_path, "en")

words = pickle.load(open('en.pickle', 'r'))

for filename in get_all_files("/scratch-local/users/dhruvils/Part2-TextExtraction/data/articles/en"):
# for filename in get_all_files("/scratch-local/users/dhruvils/Part3-NamesMarkup/data/articles/en"):
	file_exists = markup_file(filename, backlink_full, backlink_set, "en", words)
	if file_exists:
		file_count += 1
		if (file_count > 12500):
			break